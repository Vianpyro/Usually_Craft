...
execute store result score ground_coins_count buffer if entity @e[type=interaction,tag=ground_coin_pouch_interaction]

# Stop the function if the number of coins is greater than the maximum
execute if score ground_coins_count buffer >= .max_ground_coin_pouches buffer run return fail
...
execute as @e[type=minecraft:interaction,tag=ground_coin_pouch_placing] at @r[predicate=spawn:in_spawn] run function easter_egg:spawn/coins_pouch/place
