...
execute if score @s last_daily_reward_day = day buffer run function main:message/generic/already_claimed_daily_reward
execute if score @s last_daily_reward_day = day buffer run return fail

# Set the last claimed day to the current day
scoreboard players operation @s last_daily_reward_day = day buffer
