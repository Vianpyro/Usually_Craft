# Score to trigger the loto game
scoreboard objectives add lottery_trigger trigger

# Score to store the players' bids
scoreboard objectives add lottery_bids dummy

# Score to store the players' dice roll
scoreboard objectives add lottery_scores dummy

# Stats
scoreboard objectives add lottery_total_bids dummy
scoreboard objectives add lottery_play_count dummy
scoreboard objectives add lottery_total_gains dummy
scoreboard objectives add lottery_win_count dummy
scoreboard objectives add lottery_win_streak dummy
